...
# 2. ML-ReliefF算法
def ml_relief_f(X, y, k=20, m=200):
    """
    ML-ReliefF算法实现
...
# 3. 特征选择
def select_features_by_ml_relief_f(X, y, feature_columns, k=100):
    """
    使用ML-ReliefF方法选择特征
    :param X: 特征矩阵 (n_samples, n_features)
    :param y: 标签矩阵 (n_samples, n_labels)
    ...
    selected_weights = sorted_weights[:k]
...
if __name__ == "__main__":
    ...
    # 特征选择
    k = 100  # 选择的特征数量
    selected_indices, selected_weights = select_features_by_ml_relief_f(X, y, feature_columns, k)

    # 输出结果
    print("\n选择的特征索引:", selected_indices)
    print("选择的特征名称:", feature_columns[selected_indices])
    print("选择的特征权重:", selected_weights)