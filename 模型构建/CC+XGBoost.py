...
# 5. 保存结果到Excel
def save_results_to_excel(all_fold_metrics, best_params, filename='XGBoost_CC_results.xlsx'):
    """将结果保存到Excel文件"""
    # 创建结果DataFrame
...
def main():
    ...
    # 定义XGBoost基础分类器
    base_xgb = XGBClassifier(random_state=42, n_jobs=-1, eval_metric='logloss', use_label_encoder=False)

    # 定义分类器链
    cc = ClassifierChain(base_xgb, order='random', random_state=42)
