...
# 5. 保存结果到Excel
def save_results_to_excel(all_fold_metrics, best_params, filename='RF_CC_results.xlsx'):
    """将结果保存到Excel文件"""
    # 创建结果DataFrame
...
def main():
    # 加载数据
    file_path = "D:\\研二2\\论文撰写\\数据合并\\标准化、缺失值\\不填充缺失值\\SMOTE.xlsx"
    ...
    # 定义分类器链
    cc = ClassifierChain(base_rf, order='random', random_state=42)
