...
class HOMER(BaseEstimator, ClassifierMixin):
    ...
    def fit(self, X, y):
        ...
        for label_idx in self.label_order:
            ...
            # 将当前标签的预测结果添加到特征中
            if len(self.models_) < n_labels:  # 最后一个标签不需要添加
                # 对于SVM，使用decision_function而不是predict_proba
                if hasattr(model, 'decision_function'):
                    pred = model.decision_function(X_transformed).reshape(-1, 1)
                else:
    ...
    def predict(self, X):
        ...
        for i, label_idx in enumerate(self.label_order):
            ...
            # 将当前标签的预测结果添加到特征中
            if i < self.n_labels_ - 1:  # 最后一个标签不需要添加
                # 对于SVM，使用decision_function而不是predict_proba
                if hasattr(model, 'decision_function'):
                    pred_proba = model.decision_function(X_transformed).reshape(-1, 1)
                else:
    ...
    def predict_proba(self, X):
        ...
        for i, label_idx in enumerate(self.label_order):
            model = self.models_[i]
            # 对于SVM，将decision_function的输出转换为概率形式
            if hasattr(model, 'decision_function'):
                prob = model.decision_function(X_transformed)
                # 使用sigmoid函数将decision_function输出转换为概率
            ...
            if i < self.n_labels_ - 1:  # 最后一个标签不需要添加
                if hasattr(model, 'decision_function'):
                    prob = model.decision_function(X_transformed).reshape(-1, 1)
                else:
...
# 5. 保存结果到Excel
def save_results_to_excel(all_fold_metrics, best_params, filename='HOMER_SVM_results.xlsx'):
    """将结果保存到Excel文件"""
    # 创建结果DataFrame
...
def main():
    ...
    # 定义HOMER模型
    homer = HOMER(base_svm, random_state=42)

    # 定义SVM超参数网格
    param_grid = {
        'base_estimator__C': [10],
        'base_estimator__kernel': ['rbf'],
        'base_estimator__gamma': ['scale', 0.001, 0.01, 0.1],
        'base_estimator__degree': [3]  # 仅用于多项式核
    }

    print("开始HOMER+SVM网格搜索...")
